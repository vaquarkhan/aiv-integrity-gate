    ...
    public AIVConfig getConfig(Path workspace) {
        ...
        for (int i = 0; i < gates.size(); i++) {
            if ("doc-integrity".equals(gates.get(i).getId())) {
                Map<String, Object> cfg = new HashMap<>(gates.get(i).getConfig());
                cfg.put("auto", false);
                gates.set(i, new AIVConfig.GateConfig("doc-integrity", true, cfg));
                found = true;
                break;
            }
        }
        if (!found) {
            gates.add(new AIVConfig.GateConfig("doc-integrity", true, Map.of("rules_path", ".aiv/doc-rules.yaml", "auto", false)));
        }
