...
public final class InitCommand {
    private static final int DETECTION_MAX_DEPTH = 20;
    ...
    private static Set<String> detectLanguages(Path workspace) throws IOException {
        ...
        detectFromBuildFiles(workspace, found);
        Files.walkFileTree(workspace, EnumSet.noneOf(java.nio.file.FileVisitOption.class), DETECTION_MAX_DEPTH,
    ...
    private static void detectFromBuildFiles(Path workspace, Set<String> found) {
        if (Files.exists(workspace.resolve("pom.xml")) || Files.exists(workspace.resolve("build.gradle"))
                || Files.exists(workspace.resolve("build.gradle.kts"))) {
            found.add("java");
        }
        if (Files.exists(workspace.resolve("requirements.txt")) || Files.exists(workspace.resolve("pyproject.toml"))) {
            found.add("python");
        }
