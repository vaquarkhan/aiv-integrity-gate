...
public final class DesignComplianceGate implements QualityGate {
    ...
    public GateResult evaluate(AIVContext context) {
        ...
        for (ChangedFile file : context.getDiff().getChangedFiles()) {
            ...
            String rawSurface = content == null ? "" : content.toLowerCase();
            for (DesignRules.Constraint c : rules.getConstraints()) {
                // Attribution markers often live in comments/strings; do not strip those for this constraint.
                boolean attributionConstraint = c.getId() != null
                        && c.getId().toLowerCase().contains("ai-generated");
                String surface = attributionConstraint ? rawSurface : ruleSurface;
                boolean applies = c.getKeywords().isEmpty()
                        || c.getKeywords().stream().anyMatch(k ->
                        containsPattern(surface, k) || containsPattern(pathLower, k));
                if (!applies) {
                    continue;
                }
                for (String forbidden : c.getForbiddenCalls()) {
                    if (containsPattern(surface, forbidden)) {
                ...
                for (String required : c.getRequiredCalls()) {
                    if (!containsPattern(surface, required)) {
    ...
    private static Pattern phrasePattern(String pattern) {
        ...
        if (parts.length == 1) {
            // Single token (e.g. "AI-generated"): allow trailing prose.
            regex.append("(?![a-z0-9_$])");
        } else {
            // Multi-word phrase: do not match a proper prefix of a longer phrase
            // ("Generated by AI" must not hit "generated by ai doctor notes").
